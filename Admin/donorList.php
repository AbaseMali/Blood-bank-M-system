...
<div class="col-12">
		...
		while ($row = mysqli_fetch_array($donor)) {
			echo "
				<tr>
					...
					<td>{$row['unit']} </td>
					<td>	<a class=\"btn btn-success\" name=\"send\" value = \" send \" href='donor.php?did={$row['donor_id']}' onclick='return confirm(\"Are you You sure to send a lab\")'><i class=\"fa fa-send\"></i></a></td>
	...
	if (isset($_GET['did'])) {
		...
		$q = "DELETE FROM `lab`  WHERE donor_id = '$did'";
		if (!mysqli_query($conn, $q)) {
			die(" failed to delete row " . mysqli_error($conn));
		} else{
